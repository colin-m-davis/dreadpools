...
// "identity map"
template <typename T> T identity(T arg) {
...
TEST(TestPool, Lambda) {
  ...
  auto fut = pool.submit(
      [](const int a, const int b) { return std::max(a, b); }, 1, 2);
  auto expected = 2;
  auto result = fut.get();
...
TEST(TestPool, Reduce) {
  ...
  auto fut = pool.submit(std::reduce<decltype(in_vec.cbegin())>,
                         in_vec.cbegin(), in_vec.cend());
  auto result = fut.get();
